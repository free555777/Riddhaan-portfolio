...
import { 
  Mail, Phone, MapPin, ArrowRight, Layout as LayoutIcon, Smartphone, 
  ShoppingCart, Search, ShieldCheck, Database, Star, Zap, 
  Check, X, Loader2, Lock, LogOut, Settings, BarChart3, 
  MessageSquare, Globe, Sparkles, MessageCircle, Quote
} from 'lucide-react';
...
import { SiteSettings, PricingPlan, Service, Project, Inquiry, Testimonial } from './types';
...
const Portfolio = ({ items }: { items: Project[] }) => (
  <section id="portfolio" className="py-16 md:py-32 bg-white">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <SectionHeader title="Featured Work" subtitle="A section of high-end projects delivered recently" />
...
const App = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const [s, p, sv, pt, t] = await Promise.all([
          db.getSiteSettings(), db.getPricingPlans(), db.getServices(), db.getPortfolio(), db.getTestimonials()
        ]);
        if (s) setSettings(s);
        if (p && p.length) setPlans(p);
        if (sv && sv.length) setServices(sv);
        if (pt && pt.length) setPortfolio(pt);
        if (t && t.length) setTestimonials(t);
      } catch (err) {
        console.warn("Using local fallbacks.");
      }
              ...
              <div className="space-y-6 md:space-y-8 flex flex-col items-center lg:items-start">
                ...
                >
                  <motion.div
                    ...
                    className="mr-4 md:mr-6 p-3 md:p-4 rounded-full group-hover:bg-primary/20 transition-all border border-transparent group-hover:border-primary/50 group-hover:shadow-[0_0_20px_rgba(37,99,235,0.4)]"
                  >
                    <Mail className="w-6 h-6 md:w-8 md:h-8 text-primary group-hover:text-blue-400" />
                  </motion.div>
                  <span className="text-base md:text-2xl font-black break-all group-hover:text-primary transition-colors">{settings.contact_email}</span>
                </motion.div>
                ...
                >
                  <motion.div
                    ...
                    className="mr-4 md:mr-6 p-3 md:p-4 rounded-full group-hover:bg-[#25D366]/20 transition-all border border-transparent group-hover:border-[#25D366]/50 group-hover:shadow-[0_0_20px_rgba(37,211,102,0.4)]"
                  >
                    <MessageCircle className="w-6 h-6 md:w-8 md:h-8 text-[#25D366] group-hover:text-green-400" />
                  </motion.div>
                  <span className="text-base md:text-2xl font-black group-hover:text-[#25D366] transition-colors">{settings.contact_phone}</span>
                </motion.div>
              </div>
            </div>
            <div className="bg-white rounded-[24px] md:rounded-[48px] p-8 md:p-12 text-gray-900 shadow-2xl mt-8 lg:mt-0">
              <form className="space-y-6 md:space-y-8" onSubmit={handleContactSubmit}>
                <div className="space-y-6 md:space-y-8">
                  <input required name="name" type="text" placeholder="Your Name" className="w-full bg-gray-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-3.5 md:py-4 outline-none focus:ring-2 focus:ring-primary text-xs md:text-base" />
                  <input required name="phone" type="tel" placeholder="Phone Number" className="w-full bg-gray-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-3.5 md:py-4 outline-none focus:ring-2 focus:ring-primary text-xs md:text-base" />
                </div>
                <input required name="email" type="email" placeholder="Email Address" className="w-full bg-gray-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-3.5 md:py-4 outline-none focus:ring-2 focus:ring-primary text-xs md:text-base" />
                <textarea required name="message" rows={4} placeholder="Tell me about your project..." className="w-full bg-gray-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-3.5 md:py-4 outline-none focus:ring-2 focus:ring-primary resize-none text-xs md:text-base" />
                <Button fullWidth disabled={isSubmitting} className="rounded-xl md:rounded-2xl py-4 md:py-6 text-sm md:text-lg">
